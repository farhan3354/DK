  ...
  return (
    <div className="slideshow-container flex justify-center items-center bg-yellow-300 min-h-screen">
      ...
      {slidesPerPage.map((slides, index) => (
        <div
          key={index}
          className={`slide p-4 ${index === currentSet ? 'block' : 'hidden'}`}
          style={{ width: `calc(100% / ${slides})` }}
        >
          {projects.slice(0, slides).map((project, index) => (
            <div
              key={index}
              className="border-2 border-gray-200 border-opacity-60 rounded-xl overflow-hidden bg-white"
            >
              <img
