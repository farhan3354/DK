  ...
  return (
    <div className="flex flex-wrap justify-center p-4">
      {comments
        ...
        .map(comment => (
          <div key={comment.id} className="relative flex flex-col items-center w-full sm:w-64 md:w-80 p-4 m-2 rounded-lg shadow-md border-2 border-yellow-500 overflow-hidden">
            <div className="relative w-32 h-32 mb-4">
              <img 
                src={comment.image}
                alt={`Comment ${comment.id}`}
                className="absolute inset-0 w-40 h-40 object-cover rounded-full"
                style={{
                  transform: 'translate(-20%, -20%)' // Adjust the value as needed to position the image
                }}
              />
            </div>
