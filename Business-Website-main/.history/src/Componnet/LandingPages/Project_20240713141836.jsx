...
const Project = () => {
  ...
  return (
    <div className="slideshow-container flex justify-center items-center bg-yellow-300 min-h-screen max-w-xs max-h-4/5">
      {/* Map over projects array to render each project */}
      {projects.slice(0, slidesPerPage[currentSet]).map((project, index) => (
        <div
          key={index}
          className={`slide p-4`}
          style={{ width: `calc(100% / ${slidesPerPage[currentSet]})` }}
        >
          <div className="border-2 border-gray-200 border-opacity-60 rounded-xl overflow-hidden bg-white">
            <img
