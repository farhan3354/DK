...
const ProjectItem = ({ project, index, inView, setInView }) => {
  ...
  return (
    ...
    >
      {index % 2 === 0 ? (
        <>
          <img src={project.src} alt={project.alt} className='w-full md:w-1/2 h-auto mb-6 md:mb-0' border rounded-xl />
          <p className='text-sm md:ml-4'>{project.text}</p>
        </>
      ) : (
        <>
          <p className='text-sm md:ml-4'>{project.text}</p>
          <img src={project.src} alt={project.alt} className='w-full md:w-1/2 h-auto mb-6 md:mb-0' />
        </>
      )}
    </motion.div>
  );
